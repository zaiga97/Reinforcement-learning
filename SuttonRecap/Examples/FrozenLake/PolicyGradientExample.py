...
from RL_Algorithms.nn_models import LinearPolicyNN, LinearNN, SmallPolicyNN, SmallNN
...
if __name__ == '__main__':
    ...
    onehot_policy_nn = LinearPolicyNN(64, 4)
    ...
    for episode in range(episodes):
        if episode % episodes_between_tests == 0:
            ...
            #     print(f"{xy_diff_policy.get_prob(xy_env.state_coord(55))}")
            #     print(f"{xy_baseline_nn(torch.Tensor(xy_env.state_coord(0))).detach()}")
            #     print(f"{xy_baseline_nn(torch.Tensor(xy_env.state_coord(55))).detach()}")
            #     print(f"{onehot_baseline_nn(torch.Tensor(onehot_env.state_coord(0))).detach()}")
            #     print(f"{onehot_baseline_nn(torch.Tensor(onehot_env.state_coord(55))).detach()}")
